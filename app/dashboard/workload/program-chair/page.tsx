...
// ===== UTILITY FUNCTIONS =====
function determineWorkloadStatus(instructors: InstructorLoad[]): WorkloadStatus {
  ...
  // ✅ External ข้ามขั้นตอน responsible → ประธานรับรองได้เลย
...
function filterInternalCourses(allCourses: any[], chairId: string, userRole?: string): any[] {
  return allCourses.filter((c: any) => {
    const isProgramChair = String(c.program?.programChairId) === String(chairId);
  ...
  internalCourses.forEach((course: any) => {
    ...
    Object.entries(assignmentsBySemester).forEach(([semKey, semAssignments]) => {
      ...
      const relevantAssignments = semAssignments.filter((a: any) => {
        if (a.courseType === "EXTERNAL" || !!a.externalCourseCode) return false;
...
function useProgramChairData(chairId: string | undefined, userRole?: string) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const internalCourses = filterInternalCourses(allCourses, chairId, userRole);
...
function useChairApproval(onSuccess: () => void, chairId?: string) {
  ...
  const approve = useCallback(
    async (course: CourseWorkload) => {
      if (!course.instructors || course.instructors.length === 0) {
        await Swal.fire({ title: "ไม่สามารถรับรองได้", text: "ไม่พบรายชื่อผู้สอน", icon: "error", confirmButtonText: "ตกลง" });
        return;
      }

      const totalHours = course.instructors.reduce(
        (sum, i) => sum + i.lecture + i.lab + i.exam + i.critique, 0
      );
      ...
      const result = await Swal.fire({
        ...
        confirmButtonColor: "#84cc16", cancelButtonColor: "#dc2626",
        confirmButtonText: "ใช่, รับรองข้อมูล", cancelButtonText: "ยกเลิก", focusCancel: true,
      });
      ...
      try {
        ...
        if (failures.length > 0) {
          ...
          throw new Error(`อัปเดตล้มเหลว ${failures.length}/${course.instructors.length}:\n${reasons}`);
        }
        ...
        await Swal.fire({ title: "เกิดข้อผิดพลาด", text: error instanceof Error ? error.message : "บันทึกไม่สำเร็จ", icon: "error", confirmButtonText: "ตกลง" });
        onSuccess();
      } finally {
  ...
  const reject = useCallback(
    async (course: CourseWorkload) => {
      if (!course.instructors || course.instructors.length === 0) {
        await Swal.fire({ title: "ไม่สามารถส่งกลับได้", text: "ไม่พบรายชื่อผู้สอน", icon: "error", confirmButtonText: "ตกลง" });
        return;
      }
      ...
      const { value: reason } = await Swal.fire({
        ...
        input: "textarea", inputLabel: "เหตุผลในการส่งกลับ (ไม่บังคับ)",
        ...
        confirmButtonText: "ส่งกลับแก้ไข", cancelButtonText: "ยกเลิก",
        inputValidator: (v) => (v && v.length < 5) ? "กรุณาระบุอย่างน้อย 5 ตัวอักษร" : null,
      });
      ...
      try {
        ...
        if (failures.length > 0) {
          const reasons = failures.map((f) => (f as PromiseRejectedResult).reason.message).join("\n");
          throw new Error(`ส่งกลับล้มเหลว ${failures.length}/${course.instructors.length}:\n${reasons}`);
        }
        ...
        await Swal.fire({ title: "เกิดข้อผิดพลาด", text: error instanceof Error ? error.message : "ส่งกลับไม่สำเร็จ", icon: "error", confirmButtonText: "ตกลง" });
        onSuccess();
      } finally {
        setProcessingId(null);
      }
...
export default function ProgramChairPage() {
  ...
  const { courses, loading, error, refetch } = useProgramChairData(currentChair?.id, currentChair?.role);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-lime-50/20 to-emerald-50/20 p-6 font-sarabun">
      ...
      <header className="mb-8">
        <div className="flex items-center gap-2 text-slate-400 mb-2 text-sm font-medium">
          <span>จัดการชั่วโมงสอน</span><ChevronRight size={14} />
          <span className="text-lime-600 font-semibold">ประธานหลักสูตร</span>
        </div>
        <h1 className="text-4xl font-bold text-slate-800 tracking-tight mb-2">พิจารณาภาระงานสอน</h1>
        {currentChair && !loading && (
          <p className="text-slate-600 mt-2 font-light">
            ยินดีต้อนรับ, <span className="font-semibold text-lime-600">{currentChair.name}</span>
          </p>
        )}
      ...
      <div className="bg-white/90 backdrop-blur-sm p-6 rounded-2xl border border-slate-200/60 shadow-lg mb-6 sticky top-4 z-20">
        <div className="flex flex-col md:flex-row gap-4 items-center w-full">
          <div className="relative w-full flex-1 max-w-2xl">
            <label className="block text-sm font-medium text-slate-600 mb-2">ค้นหารายวิชา</label>
            <Input className="rounded-xl border-slate-200 bg-slate-50/50 pl-11 focus:ring-lime-100 h-11 w-full"
      ...
      <main>
        {loading ? (
          ...
          <div className="py-20 text-center border-2 border-dashed border-red-200 rounded-3xl bg-red-50/50">
            ...
            <button onClick={refetch} className="mt-4 px-6 py-2 bg-red-600 text-white rounded-xl hover:bg-red-700">ลองใหม่</button>
          </div>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {course.instructors.map((instructor, idx) => (
                          <tr key={instructor.id} className={`group transition-colors ${
                            instructor.isExternal ? "bg-orange-50/30 hover:bg-orange-50/60" : "hover:bg-lime-50/30"
                          }`}>
                            {idx === 0 ? (
                              <>
                                <td rowSpan={course.instructors.length + 1} className="py-6 px-6 align-top border-r border-slate-100 bg-white">
                                  <div className="flex flex-col gap-2.5">
                                    <div className="flex items-start justify-between gap-3 flex-wrap">
                                      <span className="font-bold text-xl text-slate-800 tracking-tight">{course.code}</span>
                                      <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-lime-100 text-lime-700 border border-lime-200 whitespace-nowrap">
                                        <BookOpen size={13} />{course.credit} หน่วยกิต
                                      </span>
                                    </div>
                                    <div className="flex flex-col">
                                      <span className="text-slate-700 font-medium leading-snug">{course.name}</span>
                                      {course.nameEn && <span className="text-slate-400 text-xs mt-1">{course.nameEn}</span>}
                                    </div>
                                ...
                                <td rowSpan={course.instructors.length + 1} className="py-6 px-4 align-top border-r border-slate-100 bg-white text-center">
                                  {(() => {
                            ...
                            <td className="py-4 px-6 align-middle">
                              <span className="text-slate-700 font-medium">{instructor.name}</span>
                            </td>
                            ...
                            <td className="py-4 px-3 text-center">
                              {instructor.isExternal ? (
                                ...
                                <div className="inline-flex items-start gap-1.5 px-3 py-1.5 rounded-lg text-xs bg-yellow-50 text-yellow-800 border border-yellow-200 text-left max-w-[180px]"
                        ...
                        <tr className="bg-gradient-to-r from-lime-50 to-emerald-50 border-t-2 border-lime-200">
                          <td colSpan={2} className="py-3.5 px-6 text-right text-xs font-bold text-slate-500 uppercase tracking-wide">รวมทั้งหมด</td>
                          <td className="py-3.5 px-3 text-center font-bold text-lime-600 text-lg">{course.instructors.reduce((s, i) => s + i.lecture, 0)}</td>
                          <td className="py-3.5 px-3 text-center font-bold text-lime-600 text-lg">{course.instructors.reduce((s, i) => s + i.lab, 0)}</td>
                          <td className="py-3.5 px-3 text-center font-bold text-lime-600 text-lg">{course.instructors.reduce((s, i) => s + i.exam, 0)}</td>
                          <td className="py-3.5 px-3 text-center font-bold text-lime-600 text-lg">{course.instructors.reduce((s, i) => s + i.critique, 0)}</td>
                          <td />
                        </tr>
                      </tbody>
                    </table>
                      ...
                      {course.status === "pending_approval" && (
                        <motion.div key="pending" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
                          ...
                          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                            disabled={processingId === course.id} onClick={() => approve(course)}
                            ...
                            {processingId === course.id ? <Loader2 className="animate-spin h-5 w-5" /> : <FileCheck className="h-5 w-5" />}
                            {processingId === course.id ? "กำลังบันทึก..." : "รับรองข้อมูล"}
                          </motion.button>
                          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                            disabled={processingId === course.id} onClick={() => reject(course)}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            <p className="text-slate-400 mb-4">
              {searchTerm ? "ไม่พบรายวิชาที่ตรงกับเงื่อนไขการค้นหา"
                : currentChair ? "ขณะนี้ยังไม่มีรายวิชาที่ส่งมาให้ท่านพิจารณา"
